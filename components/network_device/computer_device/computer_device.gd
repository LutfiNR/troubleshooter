...
func get_interface(interface_id: String = "eth0") -> NetworkInterface:
	return super.get_interface(interface_id)
...
func verify_configuration(runtime_device_configuration: DeviceData) -> Dictionary:
	...
	is_correct = (is_correct and gateway_result.status and dns_result.status)
