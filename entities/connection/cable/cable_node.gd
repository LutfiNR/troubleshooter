...
func _start_drag() -> void:
	if plugged_port and plugged_port.connected_cable == self:
		...
		var device: DeviceData = NetworkManager.get_runtime_device_data_by_id(
			plugged_port.device_id
		)
		var iface = device.get_interface(plugged_port.id)
		iface.state = NetworkInterface.InterfaceState.DOWN
		NetworkManager.update_interface_device_data(plugged_port.device_id, plugged_port.id, iface)
...
func _plug_into(target_port: Port) -> void:
	if target_port.is_occupied():
		...
		_unplug()
		return
	if not target_port.can_accept_cable:
		print_debug("Port tidak bisa menerima kabel!")
		_unplug()
		return
	...
	var device: DeviceData = NetworkManager.get_runtime_device_data_by_id(target_port.device_id)
	var iface = device.get_interface(target_port.id)
	iface.state = NetworkInterface.InterfaceState.UP
	NetworkManager.update_interface_device_data(target_port.device_id, target_port.id, iface)
