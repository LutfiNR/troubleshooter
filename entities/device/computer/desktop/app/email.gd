...
func _on_login_pressed() -> void:
	...
	if response.success:
		...
		if (
			server and server.handle_mail_request()
			and server.handle_mail_request().domain_name != email_parts[1]
		):
...
func _show_main_panel() -> void:
	...
	main_panel.show()
	if welcome_label:
		welcome_label.text = "Inbox - " + current_logged_in_email
...
func _load_dummy_inbox() -> void:
	...
	inbox_list.add_item("Test Koneksi")
