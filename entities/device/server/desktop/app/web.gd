...
var package_need: String = "apache2"
...
func display_data(device: ServerDevice, device_id: String) -> void:
	...
	if web_service: web_service.set_pressed_no_signal(device.web_service == ServerDevice.ServiceState.ON)
...
func _on_add_button_pressed() -> void:
	...
	new_vhost.server_name = "new.host"
	new_vhost.content = WebContent.new()
...
func _on_save_button_pressed() -> void:
	if current_selected_index == -1: return
	var vhost = current_vhosts[current_selected_index]
	vhost.server_name = server_name.text.strip_edges()
	vhost.document_root = document_root.text.strip_edges()
	vhost.protocol = protocol.selected as WebVirtualHost.Protocol
	if not vhost.content: vhost.content = WebContent.new()
	vhost.content.content = web_content.text
...
func _apply_to_server() -> void:
	...
	if web_service and web_service.button_pressed: master_state = ServerDevice.ServiceState.ON
