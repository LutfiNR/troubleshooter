...
func _populate_device(parent: TreeItem, device: Dictionary) -> void:
	var tree = _create_tree_item(parent, device["device_id"]["correct"], device["status"], true)
...
func _populate_interfaces(parent: TreeItem, interfaces: Dictionary) -> void:
	var iface_tree = _create_tree_item(parent, "Interfaces", true, true)
	var interfaces_status: bool = true
	for iface in interfaces:
		interfaces_status = interfaces_status and interfaces[iface]['status']
		_populate_interface(iface_tree, iface, interfaces[iface])
	iface_tree.set_custom_color(0, COLOR.get(interfaces_status))
...
func _populate_server_service(
	...
	match service_name:
		"DHCP":
			_populate_dhcp_service_details(service_tree, details)
		"DNS":
			_populate_dns_service_details(service_tree, details)
		"Web":
			_populate_web_service_details(service_tree, details)
		"FTP":
			_populate_ftp_service_details(service_tree, details)
		"Remote":
			_populate_remote_service_details(service_tree, details)
		"Samba":
			_populate_samba_service_details(service_tree, details)
		"MariaDB":
			_populate_mariadb_service_details(service_tree, details)
		"Mail":
